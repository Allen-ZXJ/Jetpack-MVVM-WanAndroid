...
public class HomeArticleDataSource extends PageKeyedDataSource<Integer, HomeArticle> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, HomeArticle> callback) {
        if(homeApi == null){
            homeApi = (HomeApi) ApiRetrofit.getInstance(BaseApp.context).getApiService(HomeApi.class);
        }
        homeApi.getHomeArticles(FIRST_PAGE).enqueue(new Callback<BaseRespone<BaseListData<HomeArticle>>>() {
            @Override
            public void onResponse(Call<BaseRespone<BaseListData<HomeArticle>>> call, Response<BaseRespone<BaseListData<HomeArticle>>> response) {
                ...
                LogUtils.d("Article",response.body().getData().getData().get(0).getAuthor());
            }
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, HomeArticle> callback) {
        ...
        homeApi.getHomeArticles(params.key).enqueue(new Callback<BaseRespone<BaseListData<HomeArticle>>>() {
            @Override
            public void onResponse(Call<BaseRespone<BaseListData<HomeArticle>>> call, Response<BaseRespone<BaseListData<HomeArticle>>> response) {
